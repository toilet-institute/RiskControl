package com.jxlt.udic.riskcontrol.swagger2.service.impl;

import com.jxlt.udic.riskcontrol.swagger2.dto.Request;
import com.jxlt.udic.riskcontrol.swagger2.dto.Response;
...
public class WordServiceImpl implements WordService {
    ...
    @Value("${swagger-json-uri}")
    private String swaggerUrlHead;

    @Override
    public List<Table> tableList(String groupName) {
        String swaggerUrl = swaggerUrlHead + groupName;
        ...
        String host = StringUtils.substringBefore(swaggerUrl, ":") + "://" + String.valueOf(map.get("host")) + "/udic";
        //解析paths
            ...
            while (it.hasNext()) {
                ...
                while (it3.hasNext()) {
                    ...
                    if ("200".equalsIgnoreCase(statusCode)) {
                        schema = (LinkedHashMap) statusCodeInfo.get("schema");
                    }
                ...
                String responseParam = schema == null ? "" : findRef(new JSONObject(schema), jsonObject.getJSONObject("definitions")).toJSONString();
//                table.setResponseParam(addNewLineInStr(responseParam, 10));
                table.setResponseParam(responseParam.replaceAll("[:]", " : ")
                        .replaceAll("[{]", " { ")
                        .replaceAll("[}]", " } ")
                        .replaceAll("[,]", " , "));
//                table.setResponseParam(doRestRequest(restType, buildUrl, paramMap));
    ...
    private String addNewLineInStr(String str, int num) {
    ...
    private JSONObject findRef(JSONObject jsonObject, JSONObject definitions) {
        for (String key: jsonObject.keySet()) {
            if ("$ref".equals(key)) {
                String value = jsonObject.getString(key);
                JSONObject newValue = getRefValue(value, definitions);
                jsonObject.remove(key);
                String newKey = newValue.getString("title");
                newValue.remove("title");
                jsonObject.put(newKey, newValue);
            } else {
                try {
                    findRef(jsonObject.getJSONObject(key), definitions);
    ...
    private JSONObject getRefValue(String ref, JSONObject definitions) {
        String[] refs = ref.split("[/]");
        String name = refs[refs.length - 1];
        JSONObject result = definitions.getJSONObject(name);
        return findRef(result, definitions);
    }

    private JSONObject getDeepRefValue(JSONObject result, JSONObject definitions) {
        ...
        for (String key : properties.keySet()) {
            ...
            if (value.keySet().contains("$ref")) {
                value.put("$ref", getRefValue(value.getString("$ref"), definitions));
            }
        }
        return result;
    }
